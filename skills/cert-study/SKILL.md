---
name: cert-study
description: Codex 안에서 SQLD CBT 학습 세션을 진행하고, SQLite 학습 기록과 Obsidian용 오답노트를 생성한다. Notion 동기화는 선택 기능이며 기본값은 비활성이다.
---

# Cert Study Skill

사용자가 Codex 안에서 SQLD CBT 학습, 문제 풀이, 오답노트, 복습 세션을 요청하면 이 skill을 사용한다.
...
2. 사용자가 문제 풀이를 요청하면 먼저 과목이 실제 지원되는지 판단한다.
   ...
   - 공개 기본 과목은 `SQLD`다.
   - 추가 과목은 `list_exams`의 `available`에 있는 경우에만 시작한다.
   - 아직 bank가 없는 과목은 임의로 문제를 만들지 않는다.

3. 사용자가 `SQLD 문제 시작`, `SQLD 20문제`, `SQLD 시험 문제 내줘`라고 말하면 필요한 경우에만 확인한다.
...
5. 문제는 한 번에 하나만 보여준다.
6. 사용자가 답하면 `submit_answer`를 호출하거나 아래 명령을 실행한다.
   ...
   - 단일정답은 `3`처럼 입력한다.
   - 복수정답은 `1,3`처럼 쉼표로 입력한다.
   - 복수정답은 정답 선택지를 모두 맞혀야 정답으로 처리한다.
...
- 유료 문제집 스캔이나 복사한 기출 덤프를 공개 repo에 가져오지 않는다.
- 공개 repo는 SQLD 데모 문항과 학습 하네스 구현을 보여주는 용도다.
