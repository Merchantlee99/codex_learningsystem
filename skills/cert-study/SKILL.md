...
python3 -m cert_study bank convert-kdata --exam SQLD private_banks/raw_sources/sqld private_banks/import_ready/sqld/sqld_source_backed.json
...
python3 -m cert_study bank convert-kdata --exam ADSP private_banks/raw_sources/adsp private_banks/import_ready/adsp/adsp_source_backed.json
...
python3 -m cert_study bank convert-info-processing private_banks/raw_sources/info_processing/sinagong private_banks/import_ready/info_processing/ipe_past_exams.json
...
- SQLD/ADSP 텍스트/PDF/HTML 원천은 먼저 `inspect-kdata`로 문항/정답 후보 수를 확인하고, `convert-kdata`로 ignored private JSON을 만든 뒤 `--private`로 import한다.
- 정보처리기사 ZIP/PDF는 먼저 `inspect-info-processing`으로 후보 구조를 확인하고, `convert-info-processing`으로 ignored private JSON을 만든 뒤 `--private`로 import한다.
