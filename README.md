...
그래서 이 프로젝트는 웹앱을 만들지 않고, Codex 대화창을 시험 인터페이스로 쓰는 방식으로 만들었습니다. 사용자는 채팅에서 답하고, 로컬 Python 엔진은 세션, 채점, 오답, 복습 큐를 SQLite에 기록합니다. 세션이 끝나면 사람이 읽을 수 있는 Markdown 리포트와 Obsidian에서 바로 열 수 있는 오답노트가 생성됩니다.

이 레포는 공개 포트폴리오용 코드베이스이면서 설치 가능한 Codex plugin 형태를 갖습니다. 개인 학습 DB, 실제 풀이 기록, Obsidian vault Markdown 결과물은 git에 올리지 않도록 제외했습니다. 포함된 SQLD 문항은 상업 문제집이나 실제 시험 덤프가 아니라, 시스템 동작을 검증하기 위한 synthetic 훈련 문항입니다.
...
> Codex plugin이 CBT 인터페이스와 MCP 도구를 제공하되, 학습 상태와 채점은 로컬 엔진이 deterministic하게 관리하고, Obsidian/Markdown을 기본 오답노트 projection으로 사용한다. Notion은 사용자가 선택할 때만 켜는 선택 동기화 대상이다.
...
flowchart TD
  ...
  F --> G["Obsidian Vault Notes"]
  E --> H["Review Queue"]
  H --> G
  G --> B
  F --> I["Optional Disabled-by-default Notion Plan"]
```
...
| MCP server | `cert_study/mcp_server.py` | `start_session`, `submit_answer`, `finish_session`, 선택적 `prepare_notion_sync` 도구 제공 |
| CLI entrypoint | `cert_study/cli.py` | `init`, `session start`, `answer`, `finish`, `report`, 선택적 `notion plan` 명령 제공 |
...
| Obsidian exporter | `cert_study/obsidian.py` | vault-friendly 세션 노트, 개념 노트, 복습 큐 생성 |
| Notion sync harness | `cert_study/notion_sync.py` | 선택 기능. 기본 비활성 상태에서 Notion write plan만 생성 |
...
flowchart LR
  ...
  G --> I["Optional user-enabled Notion MCP write plan"]
```
...
| Obsidian Vault | 사람이 읽고 탐색하는 기본 오답노트 projection |
| Notion MCP | 선택적 동기화 대상. 원장이 아니라 보조 database view |
...
Notion은 선택 기능입니다. `Study Sessions`, `Wrong Questions`, `Concept Reviews` DB로 정리할 수 있지만, 공개 기본값에서는 실제 Notion 쓰기를 하지 않고 어떤 페이지와 row를 만들지 사용자가 검토할 수 있는 plan만 생성합니다.
...
| Obsidian export | 세션 노트, 개념별 오답 노트, 복습 큐가 `obsidian_vault/`에 생성됨 |
...
| Eval / harness thinking | 채점, 세션 진행, 리포트 생성, Obsidian export, Notion sync plan 테스트 |
...
| Knowledge workflow | Obsidian Markdown을 기본 오답노트 projection으로 두고 Notion MCP는 선택 기능으로 격리 |
...
| `prepare_notion_sync` | Optional. Generate a disabled-by-default Notion write plan |

Obsidian Markdown is the default notebook projection. To write directly into an existing vault:
