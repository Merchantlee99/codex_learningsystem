...
SQLD/ADSP처럼 텍스트, PDF, HTML로 정리된 로컬 원천이 있다면 KDATA 계열 변환기를 씁니다. 문항 본문, 보기 4개, 정답표를 추출해서 내부 CBT용 JSON으로 바꾸고, 변환 직후에는 `needs_review` 상태로 둡니다. 공식 범위와 정답을 대조한 자료만 `--mark-active`로 승격합니다.
...
정보처리기사처럼 ZIP/PDF 원천이 있는 과목은 먼저 private archive inspector로 후보 구조를 확인한 뒤 import-ready JSON으로 변환합니다. 변환 결과는 ignored 된 `private_banks/` 안에만 두고, 공개 repo에는 원문이나 변환된 문제를 올리지 않습니다.
...
| --- | --- | --- | --- |
| `SQLD` | SQLD | 국내 데이터/IT | 공개 합성 문제은행 있음, private 텍스트/PDF/HTML 변환기 있음 |
| `ADSP` | ADsP | 국내 데이터/IT | 공개 합성 문제은행 있음, private 텍스트/PDF/HTML 변환기 있음 |
| `KR_INFO_PROCESSING_ENGINEER` | 정보처리기사 | 국내 데이터/IT | 공개 합성 문제은행 있음, private ZIP/PDF inspector와 변환기 있음 |
...
2. SQLD/ADSP는 `inspect-kdata`와 `convert-kdata`로 개인 로컬 원천을 import-ready JSON으로 바꿉니다.
3. 개인이 가진 실제 공부 자료는 `private_banks/`로 가져와 로컬에서만 씁니다.
4. AWS/GCP는 공개 repo에 원천 문항을 넣지 않고, 로컬 `private_banks/`에서 허용 라이선스 자료만 변환/import합니다.
5. 과목이 더 늘어나면 Python seed를 계속 늘리기보다 JSON/YAML 데이터팩을 기본 입력으로 삼습니다.
6. 새 과목을 추가할 때마다 작은 CBT 세션을 돌리는 공통 하네스 테스트를 추가합니다.
