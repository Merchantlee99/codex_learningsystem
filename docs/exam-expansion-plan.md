...
이 공개 레포는 문제은행 보관소가 아니라 사용자가 자기 과목을 붙여 확장할 수 있는 학습 플러그인 견본입니다.
...
| 위치 | 역할 |
| --- | --- |
| 공개 repo | SQLD 합성 데모 seed, 학습 엔진, MCP/CLI, Obsidian/Notion 하네스, 테스트 |
| 로컬 작업 폴더 | 사용자 문제은행, import-ready JSON, 실제 풀이 기록, Obsidian 산출물 |
...
공개 repo의 역할은 “이 구조로 자기 학습 플러그인을 만들 수 있다”를 보여주는 것입니다. 따라서 다른 사용자가 과목을 추가할 때도 먼저 `examples/private_bank.example.json`을 복사해 자기 데이터팩을 만들고, `private_banks/`에서 로컬 import하는 흐름을 권장합니다.
